...
**Objective:** Create an HTML interface which upon loading receives information from two API sources, one which is dependent on the other. The interface should also be updated via the click of an element within the interface.
...
Do NOT use any `async / await` to do this assignment, You can only use the `Fetch` API. **Doing so may result in a zero.**
...
Style your project to have an easy to use layout and a visual aesthetic you find fitting for the topic at hand. Don't forget to link a stylesheet to your HTML!
...
Upon completion of the assignment, please link to your GitHub repo along with uploading a `.zip` of the most recent work pushed to the GitHub repo. Any commits to your repo after the assignment due date will be ignored unless otherwise discussed with your isntructor.
